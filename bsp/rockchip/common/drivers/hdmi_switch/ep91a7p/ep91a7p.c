...
static rt_err_t _ep91a7p_write_regs(struct rt_i2c_client *client, void *write_buf, rt_uint8_t write_len)
{
...
static rt_err_t ep91a7p_write_regs(rt_uint8_t u8Offset, rt_uint8_t u8ByteNo, rt_uint8_t *pu8Data)
{
    ...
    ret = _ep91a7p_write_regs(g_ep91a7p_dev->i2c_client, data_buf, u8ByteNo + 1);

    rt_free(data_buf);
    return ret;
}

static rt_err_t  _ep91a7p_read_regs(struct rt_i2c_client *client, rt_uint8_t cmd, rt_uint8_t read_len, rt_uint8_t *read_buf)
{
...
{
    return _ep91a7p_read_regs(g_ep91a7p_dev->i2c_client, cmd, read_len, read_buf);
}
...
static rt_err_t ep91a7p_power_mode_ctrl(struct hdmi_repeat *rpt, void *data)
{
    ...
    default:
        ...
        //the i2c. The first i2c command may be lost, so we send it several
        //times to ensure successful awakening.
        for (i = 0; i < 5; i++)
        {
...
static void ep91a7p_dump()
{
    ...
    {
        rt_kprintf("Power[%d] %s\n", (g_ep91a7p_dev->gc.ctl & EP_GC_POWER_MASK) >> EP_GC_POWER_SHIFT,
                   g_ep91a7p_dev->chip_mode == 0 ? "normal operation mode" : "isp mode");
...
static rt_err_t ep91a7p_fw_update(struct hdmi_repeat *rpt, void *data);
...
#ifdef RT_EP91A7P_FW_UPGRADE
        ep91a7p_fw_update(rpt, args);
#endif
        break;
    default:
...
static rt_uint8_t const ep_id[][5] =
{
    {EP_91A7P, EP91A7P_VENDOR_ID_0, EP91A7P_VENDOR_ID_1, EP91A7P_DEVICE_ID_0, EP91A7P_DEVICE_ID_1},
};

#define EP_ID_MAX    (sizeof(ep_id)/sizeof(ep_id[0]))

static rt_uint8_t ep91a7p_chip_match(void)
{
    rt_uint8_t  temp[4];
    rt_uint8_t  count;
    ...
    ep91a7p_read_regs(EP_VENDOR_ID_1, 4, temp);

    rt_kprintf("EP id: %02X %02X %02X %02X\n", temp[0], temp[1], temp[2], temp[3]);

    for (count = 0; count < EP_ID_MAX; count++)
    {
        if ((temp[0] == ep_id[count][1]) && (temp[1] == ep_id[count][2])
                && (temp[2] == ep_id[count][3]) && (temp[3] == ep_id[count][4]))
        {
            return ep_id[count][0];
        }
    }
...
static rt_err_t ep91a7p_connectivity_check(void)
{
    ...
    for (trycount = 0; trycount < 30; trycount ++)
    {
        ...
        {
            chip_id = ep91a7p_chip_match();
            if (chip_id == EP_DISMATCH)
            {
                return -RT_ERROR;
            }
            else
...
#ifdef RT_EP91A7P_FW_UPGRADE
static rt_err_t ep91a7p_isp_write_regs(rt_uint8_t u8Offset, rt_uint8_t u8ByteNo, rt_uint8_t *pu8Data)
{
    char *data_buf = RT_NULL;
    rt_err_t ret;
    data_buf = (char *)rt_calloc(1, u8ByteNo + 1);
    ...
    ret = _ep91a7p_write_regs(g_ep91a7p_dev->i2c_isp, data_buf, u8ByteNo + 1);

    rt_free(data_buf);
    return ret;
}

static rt_err_t  ep91a7p_isp_read_regs(rt_uint8_t cmd, rt_uint8_t read_len, rt_uint8_t *read_buf)
{
    return _ep91a7p_read_regs(g_ep91a7p_dev->i2c_isp, cmd, read_len, read_buf);
}

static rt_err_t ep91a7p_isp_i2c_init(void)
{
    rt_err_t ret = RT_ERROR;
    rt_uint8_t temp;
    g_ep91a7p_dev->i2c_isp = (struct rt_i2c_client *)rt_calloc(1, sizeof(struct rt_i2c_client));
    if (g_ep91a7p_dev->i2c_isp == RT_NULL)
    {
        return RT_ERROR;
    }

    g_ep91a7p_dev->i2c_isp->client_addr = EP91A7P_ISP_ADDR;
    g_ep91a7p_dev->i2c_isp->bus = (struct rt_i2c_bus_device *)rt_device_find(EP91A7P_I2C_DEV);
    if (g_ep91a7p_dev->i2c_isp->bus == RT_NULL)
    {
        rt_free(g_ep91a7p_dev->i2c_isp);
        return RT_ERROR;
    }

    ret = ep91a7p_isp_read_regs(0x02, 1, &temp);

    return ret;
}

static int ep91a7p_get_version(rt_uint8_t *version)
{
    return ep91a7p_read_regs(EP_VERSION_VER, 1, version);
}
...
static int fw_version_compare(char *fw_file_name)
{
    ...
    {
        ep91a7p_get_version(&chip_version);
        rt_kprintf("chip fw version:%x\n", chip_version);
    }
    ...
    if (fp)
    {
        ...
        if (ret != 1)
        {
            ...
            if (!ret)
            {
                ret = fread(&fw_version, 1, sizeof(fw_version), fp);
                if (ret == sizeof(fw_version))
                {

                    rt_kprintf("file fw version:%x\n", fw_version);
                    if (fw_version > chip_version)
                    {
                        ret = 1;
                    }
                    else
...
static rt_uint8_t GenCheckSum(rt_uint8_t *data, rt_uint32_t Length)
{
...
static rt_err_t fw_update_init(ISP_CTX *isp, char *fw_file_name)
{
    ...
    if (fp)
    {
        ...
        if (!ret)
        {
            isp->fw_length = ftell(fp);
            if (isp->fw_length == -1)
            {
                fclose(fp);
                return RK_REPEAT_ISP_BAD_FW;
            }
            isp->fw_code = (rt_uint8_t *)malloc(isp->fw_length);
            if (!isp->fw_code)
            {
                fclose(fp);
                return RK_REPEAT_ISP_BAD_FW;
            ...
            {
                checksum = GenCheckSum(isp->fw_code, isp->fw_length);
                rt_kprintf("checksum %x\n", checksum);
...
static rt_err_t ep91a7p_enter_isp(void)
{
    rt_uint8_t data = 0x40;

    return ep91a7p_write_regs(0x0f, 1, &data);
}

static rt_err_t ep91a7p_exit_isp(void)
{
    rt_uint8_t ret;
    rt_uint8_t data = 0x00;

    ret = ep91a7p_isp_write_regs(0x02, 1, &data);
    rt_thread_mdelay(2500);

    return ret;
}

static rt_err_t ep91a7p_update_process(ISP_CTX *isp)
{
    rt_uint32_t block_index, block_size, block_offset;
    rt_uint8_t temp[17];
    rt_uint8_t ret = RT_ERROR;

    block_size = isp->fw_length / 512;
    if ((isp->fw_length % 512) != 0)
    {
        ++block_size;
    }
    rt_kprintf("fw size %d,total %d block needs to be update\n", isp->fw_length, block_size);
    rt_thread_mdelay(10);
    for (block_index = 0; block_index < block_size; ++ block_index)
    {
        ...
        //sest block index
        temp[0] = block_index;
        ret = ep91a7p_isp_write_regs(0x01, 1, temp);
        if (ret)
        {
            break;
        }
        ...
        {
            temp[0] = 16;
            memcpy(temp + 1, isp->fw_code + (block_index * 512) + (block_offset * 16), 16);
            ret = ep91a7p_isp_write_regs(0x41, 17, temp);
            if (ret)
            {
                break;
            }
    ...
    if (ret)
    {
        rt_kprintf("ep91a7p update failed\n");
    }
    else
    {
        rt_kprintf("ep91a7p update done\n");
    }
...
static rt_err_t ep91a7p_update(struct rk_rpt_isp_op *op)
{
    rt_err_t ret;
    ISP_CTX isp = {};
    rt_uint8_t chip_id;
    ...
    if (!ret)
    {
        ...
        {
            rt_kprintf("EP91A7P FW update begin:\n");
            ret = ep91a7p_enter_isp();
            g_ep91a7p_dev->dev_status = RK_REPEAT_ISP;
            g_ep91a7p_dev->init = 0;
            if (!ret || g_ep91a7p_dev->chip_mode)
            {
                rt_thread_mdelay(500);
                ret = ep91a7p_isp_i2c_init();
                if (!ret)
                {
                    g_ep91a7p_dev->chip_mode = EP91A7P_ISP_MODE;
                    ret = ep91a7p_update_process(&isp);
                    if (ret)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            rt_kprintf("EP91A7P update success\n");
                            g_ep91a7p_dev->chip_mode = EP91A7P_NORMAL_MODE;
                            g_ep91a7p_dev->dev_status = RK_REPEAT_INIT;
                            ret = RK_REPEAT_ISP_SUCCESS;
                            rt_sem_release(g_ep91a7p_dev->isr_sem);
                        }
                    }
                }
                else
                {
                    rt_kprintf("EP91A7P isp i2c init failed\n");
                    ret = RK_REPEAT_ISP_UPDATE_FAILED;
                }
            }
            else
            {
                rt_kprintf("EP91A7P enter isp mode failed\n");
                ret = RK_REPEAT_ISP_UPDATE_FAILED;
            }
        }
    }
    ...
    {
        rt_kprintf("EP91A7P FW update init failed\n");
    }
...
static rt_err_t ep91a7p_fw_update(struct hdmi_repeat *rpt, void *data)
{
    ...
    if (g_ep91a7p_dev->dev_status == RK_REPEAT_NOT_PRESENT)
    {
        return -RT_ERROR;
    }

    ret = ep91a7p_update(op);
...
static void ep91a7p_thread(void *arg)
{
    ...
    while (1)
    {
        if (g_ep91a7p_dev->chip_mode == EP91A7P_ISP_MODE)
        {
            g_ep91a7p_dev->dev_status = RK_REPEAT_ISP;
            rt_thread_mdelay(20);
        }
...
static int rt_hw_ep91a7p_init(void)
{
    ...
    /* i2c interface bus */
    dev->i2c_client = (struct rt_i2c_client *)rt_calloc(1, sizeof(struct rt_i2c_client));
    RT_ASSERT(dev->i2c_client != RT_NULL);

    dev->i2c_client->client_addr = EP91A7P_I2C_ADDR;
    dev->i2c_client->bus = (struct rt_i2c_bus_device *)rt_device_find(EP91A7P_I2C_DEV);
...
#ifdef RT_EP91A7P_FW_UPGRADE
    rt_kprintf("ep91a7p update <op> <fw path>      -op: 1.nornal 2.force\n");
#endif
}
...
void ep91a7p(int argc, char **argv)
{
    ...
    else if (argc == 4)
    {
        ...
        else if (!strcmp(argv[1], "update"))
        {
            ...
            rt_device_control(ep91a7p_dev, RK_REPEAT_CTL_FW_UPDATE, (void *)&isp_op);
        }
#endif
