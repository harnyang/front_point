...
static void opp_low_temp_adjust(struct rk_opp_table *opp_table, int temp)
{
    ...
    {
        rt_mutex_take(&opp_table->lock, RT_WAITING_FOREVER);
        if (opp_table->is_suspended)
        {
            rt_mutex_release(&opp_table->lock);
            return;
        }
        opp_table->is_low_temp = is_low_temp;
        if (opp_set_rate_nolock(opp_table, opp_table->cur_freq) != RT_EOK)
        {
...
static int opp_pm_suspend(const struct rt_device *device, rt_uint8_t mode)
{
    struct rk_opp_table *opp_table;
    rt_err_t error = RT_EOK;

    RT_ASSERT(device != RT_NULL);
    opp_table = (struct rk_opp_table *)device->user_data;

    rt_mutex_take(&opp_table->lock, RT_WAITING_FOREVER);
    opp_table->is_low_temp = true;
    opp_table->is_suspended = true;
    error = opp_set_rate_nolock(opp_table, opp_table->cur_freq);
    rt_mutex_release(&opp_table->lock);
    if (error != RT_EOK)
    {
        rt_kprintf("opp suspend failed to set rate\n");
    }

    return error;
}

static void opp_pm_resume(const struct rt_device *device, rt_uint8_t mode)
{
    struct rk_opp_table *opp_table;
    ...
    opp_table->is_suspended = false;
}

static struct rt_device_pm_ops opp_pm_ops =
{
    .suspend = opp_pm_suspend,
    .resume = opp_pm_resume,
};
#endif
...
#if defined(RT_USING_PM)
        opp_table->parent.user_data = opp_table;
        rt_pm_device_register(&opp_table->parent, &opp_pm_ops);
#endif
        opp_table++;
    }
...
void opp_dump(int argc, char **argv)
{
    ...
    {
        rt_mutex_take(&opp_table->lock, RT_WAITING_FOREVER);
        rt_kprintf("%s: %u(Hz) %u(uV)\n", opp_table->name,
                   (uint32_t)opp_table->cur_freq, opp_table->cur_volt);
        for (i = 0; i < opp_table->num_opps; i++)
        {
            if (opp_table->is_low_temp && opp_table->low_temp_min_volt &&
                    (opp_table->opps[i].volt < opp_table->low_temp_min_volt))
...
void opp_set(int argc, char **argv)
{
    struct rk_opp_table *opp_table = g_opp_table;
    ...
    if (argc != 3)
    {
        rt_kprintf("opp_set <name> <value>\n");
        return;
    }

    while (opp_table->name)
    {
        if (strstr(opp_table->name, argv[1]))
        {
            rt_kprintf("set %s %d\n", argv[1], atoi(argv[2]));
            opp_set_rate(opp_table, atoi(argv[2]));
            return;
        }
        opp_table++;
    }
...
MSH_CMD_EXPORT(opp_set, opp driver set rate. e.g: opp_set <name> <value>);
#endif
